...
function reset(pool){
  ...
  STATE={ users:[{name:"小葵",role:"editor",craft:"both",pwHash:H,pwAt:"2020-01-01T00:00:00"},
                 ...
                 {name:"管理員",role:"boss"},{name:"Regina",role:"manager"}],
...
// ══ 搜尋跟快選分類疊在一起 ══
reset(); as("Regina","manager"); setPoolFilter("tw"); setPoolQ("珠寶");
ok("分類＋搜尋一起套用", JSON.stringify(inPool(viewWork()))===JSON.stringify(["P1","P3"]));
reset(); as("Regina","manager"); setPoolFilter("shopee"); setPoolQ("珠寶");
...
reset(); as("Regina","manager"); setPoolQ("找不到");
...
reset(); as("Regina","manager"); calls.length=0; setPoolQ("珠寶");
...
{ const w=viewWork();
  ...
  ok("提示要告訴他被指派的在哪裡", w.includes("每日工作"), (w.match(/目前沒有可以認領[^<]*/)||[])[0]); }
