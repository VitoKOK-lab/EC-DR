...
ok("editor: cal hub, zones integrated into work (v37)", JSON.stringify(tabs)===JSON.stringify(["chat","work","team","videos","videosDF","cal"]), tabs);
localStorage.setItem("ecdr_user","Anna"); localStorage.setItem("ecdr_role","intl");
tabs=myTabs().map(t=>t[0]);
ok("intl: same tab ids as editor (v37)", JSON.stringify(tabs)===JSON.stringify(["chat","work","team","videos","cal"]), tabs);
