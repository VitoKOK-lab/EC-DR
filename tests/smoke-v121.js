...
// ══════════ ③ 同仁來訊（人資／主管）：說 N 則等你處理，就要有 N 個回覆框 ══════════
// v183：這一段搬到「傳訊息」了（老闆：「找主管，找hr也是一種溝通呀，全部移到
// 溝通去」）。要守的事一個字都沒變 —— 畫面上寫幾則，就要真的有幾個回覆框。
reset(); as("HR小姐","hr"); COMM_TAB="open";
{ const c=viewChat();
  const m=c.match(/class="fn warn">(\d+)</);
  const boxes=count(c,/id="mr_M\d+"/g);
  ok("等你處理的數字抓得到", !!m);
  ok(`${m&&m[1]} 則等你處理 → 就有 ${boxes} 個回覆框`, !!m && boxes===+m[1]);
  ok("而且真的超過原本 20 的上限", boxes>20);
  ok("**看板上不再有這張卡**", !viewTeam().includes("同仁來訊")); }
...
// ══════════ ⑤ 找主管／人資說過的話：自己發過的都要看得到 ══════════
// v183：一樣搬到「傳訊息」。發訊的那一頭不會因為換地方就少掉幾則。
reset(); as("小葵","editor"); COMM_TAB="open";
{ const c=viewChat();
  const mine=myMsgs().length;
  const rows=count(c,/msgDel\('M\d+'\)|msgSeen\('M\d+'\)/g);
  ok(`自己發過 ${mine} 則 → 畫面上就有 ${rows} 則`, rows===mine, {發過:mine, 畫出來:rows});
  ok("而且真的超過原本 12 的上限", rows>12);
  ok("**每日工作上一則都沒有**", count(viewWork(),/msgDel\('M\d+'\)|msgSeen\('M\d+'\)/g)===0); }
...
{ let wrote=[]; // 只收 tasks —— logA 也走 DB.set，會把操作紀錄一起塞進來
  ...
  remindBossShoot().then(()=>new Promise(r=>setTimeout(r,30))).then(()=>{
    ...
    { const w2=viewWork();
      ...
      // v183：訊息類的卡片全部搬到「傳訊息」了，所以這一頁上一張都不該有。
      ok("**每日工作上沒有任何訊息卡**",
         at("找主管／人資說一件事")<0 && at("傳訊息給同事")<0 && at("同事來訊")<0
         && at("交辦追蹤")<0,
         {找主管:at("找主管／人資說一件事"), 傳同事:at("傳訊息給同事"),
          同事來訊:at("同事來訊"), 交辦追蹤:at("交辦追蹤")}); }
