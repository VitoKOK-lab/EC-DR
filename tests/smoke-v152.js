...
function reset(videos, who, role){
  modalHTML=""; viewEl.innerHTML=""; fields={}; OUT_FILTER="all"; TEAM_YM=null; VIEW_AS=null; BRAND="";
...
const cards=(h)=>{ const i=h.indexOf('<div class="card">'); return i<0?"":h.slice(i); };
...
{ reset(SET(), "管理員","boss");
  ...
  ok("小葵這個月完成 4 支（上上個月那支不算、還在剪的不算）", h.includes("完成 4 支"), h.match(/完成 \d+ 支/g));
  ok("阿哲這個月完成 2 支", h.includes("完成 2 支"));
  ok("小葵：審過 3", /審過 3/.test(h));
  ok("小葵：還沒審 1", /還沒審 1/.test(h));
  ok("阿哲：退回 1", /退回 1/.test(h));
  ok("標出缺資料夾的支數", /缺資料夾 1/.test(h));
  ok("還在剪的那支不會出現在清單裡", !h.includes("片WIP"));
  ok("有完成日但階段還是「剪輯中」的，不算完成", !h.includes("片HALF"));
  ok("上上個月那支不會出現", !h.includes("片OLD")); }
...
{ reset(SET(), "管理員","boss");
  ...
  ok("六支裡有資料夾的五支都給得出連結（缺的那支不給）", nDrive(h)===5, nDrive(h));
...
{ reset(SET(), "管理員","boss");
  const h0=viewOutput();
  ok("篩選鈕：全部 6", /全部<\/span> <span class="vtab-n">6</.test(h0), h0.match(/vtab-n">\d+</g));
  ok("篩選鈕上的數字是所有人加起來（審過 4）", /審過<\/span> <span class="vtab-n">4</.test(h0), h0.match(/vtab-n">\d+</g));
  ok("篩選鈕：缺資料夾 1", /缺資料夾<\/span> <span class="vtab-n">1</.test(h0));
  setOutFilter("ok");
  const h1=viewOutput();
  ok("只看審過的：退回那支不見了", !/退回<\/span>/.test(cards(h1)), cards(h1).slice(0,120));
  ok("只看審過的：還沒審那支也不見了", !/還沒審<\/span>/.test(cards(h1)));
  ok("只看審過的：審過的四支都還在", (cards(h1).match(/審過<\/span>/g)||[]).length===4);
  setOutFilter("nodrive");
  const h2=viewOutput();
  ok("只看缺資料夾的：一個資料夾連結都不該有", nDrive(h2)===0, nDrive(h2));
  ok("只看缺資料夾的：阿哲那張卡說沒有符合的", h2.includes("沒有符合目前篩選"));
...
{ reset(SET(), "HR小姐","hr");
  const h=viewOutput();
  ok("人資看得到內容", h.includes("小葵") && h.includes("開資料夾"));
...
{ reset(SET(), "管理員","boss");
  const h=viewOutput();
  ok("這一頁不會寫資料庫（沒有任何寫入的呼叫）",
     !/dbUpdate\(|dbSet\(|saveVideo\(|reviewVid\(|delVid\(/.test(h)); }
...
{ reset([ v_("OLD2",{editor:"小葵", finishedAt:D(-60)+"T10:00:00"}) ], "管理員","boss");
  const h=viewOutput();
  ok("這個月沒人完成時畫得出來（不是空白也不是壞掉）",
     h.includes("剪輯成效") && h.includes("這個月還沒有完成的影片"), h.slice(0,200));
...
{ const many=[]; for(let i=0;i<30;i++) many.push(v_("M"+i,{editor:"小葵", reviewStatus:"通過", driveFolder:FAM+"-"+i}));
  ...
  ok("30 支的人：30 列一列都沒少（是捲動不是截斷）",
     (h.match(/data-label="審核"/g)||[]).length===31, (h.match(/data-label="審核"/g)||[]).length);
  ok("只有 1 支的人不用捲動框", (h.match(/keepscroll/g)||[]).length===1); }
