...
{ reset(SET(), "麗君","cs");
  ...
  // v184（老闆指定）：「這邊的數量需要以 Regina 審片完成後才算」。
  // 小葵這個月剪完 4 支，其中 1 支還在等審 → 月成效算 3 支。
  // （剪輯產出那一頁照舊列 4 支並拆成「審過 3／還沒審 1」—— 那一頁的重點就是審到哪。）
  ok("**人資的團隊看板：只算審過的（小葵 3）**", /完成上架">3</.test(hr), hr.match(/完成上架">\d+</g));
