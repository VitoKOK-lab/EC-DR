...
{ reset(); as("管理員","boss");
  ...
  // 影片成效重複了」——「大流量影片」這個分頁後來整個拿掉了（不是改名，是
  // 導覽列不會再有這個按鈕），所以這裡改成守「兩個名字都不該出現在分頁上」。
  ok("**「大流量影片」（不管新舊名字）都不在分頁清單裡了**（分頁整個拿掉，不是改名）",
     !L.some(x=>/大流$|影片庫大流|大流量影片/.test(x)), L);
  // v181 原本是寫死「不准有任何分頁含『成效』兩個字」—— 那是拿當時剛好取的名字
  // 當代理指標。真正的要求是「兩個不同的東西不能叫同一個名字」（原本兩個都叫「成效」，
  // 沒人分得出哪個是哪個）。v202 老闆把「影片流量」改名成「影片成效」之後，
  // 舊的寫法會誤報 —— 所以改成直接守那個要求。
  ok("**兩個「成效」分開了**：剪輯產出 vs meta成效",
     L.includes("剪輯產出") && L.includes("meta成效"), L);
...
{ reset(); as("管理員","boss");
  ...
  // v203：「二創」自成一頁（10 → 11）。
  // v204：老闆看了之後說「二創理論上不就該和影片成效是一樣的嗎？只是多了一個
  //       『二創』的按鍵」—— 對。二創建議整張併進影片成效的影片排行、剪輯二創成效
  //       接在它下面、進行中的二創搬去看板（那是生產面）。分頁回到 10。
  //       ⚠️ 這裡釘的是「不要再回到什麼都塞進導覽列的狀態」，不是「永遠不准新增」。
  // v210：多了「選品」（設計師選品清單）→ 11。
  //       ⚠️ 這裡釘的是「不要再回到什麼都塞進導覽列的狀態」，不是「永遠不准新增」。
  ok("**老闆的分頁不超過 11 個**（不要再回到什麼都塞進導覽列）",
     ids().length<=11 && ids().includes("assets") && ids().includes("perf"),
     {幾個:ids().length, 是:ids()});
  ok("「二創」不再是一個分頁（它就是影片成效多一顆『排二創』按鍵）",
     !ids().includes("remake"), ids());
  ok("順序：溝通 → 看板 → 其他",
