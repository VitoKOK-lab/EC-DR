...
| `role` | string | `boss`（管理員）／`manager`（經理人）／`editor`（剪輯）／`intl`（海外剪輯・全英文介面）／`cs`（客服：被交辦工作＋每日匯報，不碰影片）／`hr`（人資：只看團隊看板，純檢視、不能操作） |
...
>
> **客服（`role:"cs"`，v66 起）**：分頁只有「本日工作」與「團隊看板」。本日工作＝交辦工作卡＋下班匯報，
> 完全沒有毛片／影片／排程；常用項目換成客服的（回覆客戶訊息、訂單處理／出貨、退換貨處理、客訴追蹤、商品資訊更新）。
> 登入即打上班卡，按「下班匯報」彙整當日工作項目與處理狀況後打下班卡 —— 這就是每日工作匯報。
> Regina 在「流程中控」與「儀表板」可以交辦客服並看回報；毛片指派清單**不含**客服。
