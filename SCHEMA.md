...
| 片名 | 自動加後綴（「（蝦皮版）」） | **排片人現取**；`rawName` 沿用原片並鎖住 |
| 誰發起 | 剪輯自己挑源片、指派給自己 | **管理員／經理人**排，指派給某個剪輯 |
...
對不回來。所以 `rmkNameLock()` 在兩種情況回鎖定理由：這支是二創殼、或這支已經有二創。
畫面上 `readonly`，**存檔時再擋一次**（`saveVideo` 回存舊值）—— readonly 擋得住手滑，
擋不住 devtools 或一個開了三天的舊分頁。
