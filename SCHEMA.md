...
| `remake` | 二創 | boss, manager, editor | `remake` |
...
我會直接把大流量那一整頁直接刪掉」「現在舊的那一個大流量頁面，那邊的資料不是那麼準確」

二創本來掛在**別人的頁面**上：建議卡在「大流量影片」，剪輯成效在「影片成效」。
那一頁是**過渡期**的東西 —— 以前沒進系統的舊片直接建檔放那裡，四個欄位全是人工填的。
成效引擎接起來之後它就沒有存在的必要了，而二創長在它上面，那頁一刪就跟著陪葬。

所以二創搬成自己的分頁（`remake`），三張卡：

| 卡 | 看什麼 |
|---|---|
| 二創建議 | 挑片（成效 × 隔多久沒用 × 用過幾次）＋ 搜尋 ＋ 排二創 |
| 進行中的二創 | 排下去了還沒上片的：誰在剪、剪幾天了、上片日剩幾天 |
| 剪輯二創成效 | 做完之後：比值中位數、最好最差、他分到的原片幾天 |

「進行中」那張卡會把**上片日到了卻還沒人按「收到」**的數出來單獨喊 ——
排了沒人收等於沒排，而排片人排完就看不到後續了，這張卡就是補那個洞。

### ⚠️「大流量」是核心，被移除的只有畫面

老闆特別澄清過：「不是叫你把大流量的移除，那是我們核心要做的事情」。
找出高流量的片拿來再用，正是整套東西的目的。二創建議的候選池照樣含大流的片
（公司做過的 5 次二創**全部都在大流**）—— 搬走的是**畫面**，不是資料。

### ⚠️ 搬家不能改變誰看得到

`remake` 的職位預設值刻意跟 `df` 一模一樣（boss／manager／editor）——
二創建議本來就掛在大流量那一頁上，搬出來的時候誰看得到不能變，
不然會有人今天有、明天沒有。`tests/smoke-v202.js` 第 ① 段釘住這件事。
