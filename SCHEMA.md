...
> **還沒做（第二批）**：成效歸戶目前還是照文案比對，二創跟原片的貼文會混在一起。
> 正確做法是「原片＋它的二創算同一個家族，先配家族，再用**上片日期＋帳號**歸到某一支」，
> 並且開始存成效快照（每 3 天一點、存 30 天），這樣才有「第 30 天對第 30 天」可以比。
> 見 `tools/meta_match.py`／`tools/meta_sync.py`。
