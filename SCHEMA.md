...
## 3e. `hrchecks/{videoId}` — 人資的檢查紀錄（**只有人資看**）

文件 ID = 影片 `id`。人資按「檢查完成」時寫入；純個人紀錄，**不影響影片與任何人的流程**，可自行清除。

| 欄位 | 型別 | 說明 |
|---|---|---|
| `id` | string | = 影片 id |
| `count` | number | 檢查次數（再按一次就 +1，用來看出同一支片被重覆送） |
| `lastBy` | string | 最後檢查的人 |
| `lastAt` | string(ISO) | 最後檢查時間 |
| `title` | string | 當下的片名（快照，方便日後回看） |
| `history` | object[] | 每次檢查 `{by, at, stage}`，最多留 20 筆 |

> **重工提醒（前端即時算，不存資料庫）**：人資清單會對每支片標紅提示，條件為
> ①`count>1`（已檢查過多次）②已檢查過卻又出現在「等待審查」③曾被退回重修
> （`reviewStatus="退回"` 或有 `reviewNote`）④完成後又回到剪輯中（`stage="剪輯中"` 且有 `finishedAt`）。
...
> 點片名開影片檢視視窗，文案、商品、連結、審片狀態全部看得到。
> 看完按「✓ 檢查完成」→ **只寫進人資自己的 `hrchecks` 集合，完全不動 `videos`**，
> 剪輯與經理人的流程零影響。人資另可看儀表板 KPI，但**不能指派**工作或毛片。
