...
| `metricsHist` | object[] | 成效快照（v201） | 每筆 `{postId, d, views, comments}`；`d`＝抓取日。`metrics` 只留每則貼文的**最新**累計，這裡才留得住「上片後第 N 天是多少」—— 沒有它，二創跟原片只能拿「30 天」去比「半年」。上片超過 35 天不再記，一則最多 15 個點。詳見下方「二創流程」 |
| `ads` | object[] | 廣告花費（v214） | `tools/ads_sync.py` 自動填（老闆選 A：系統自己去跟 Meta 要）。每筆 `{adId, name, status, postId, spend, impressions, reach, clicks, purchases, since, until, at}`，一列＝**一則廣告在一個區間**；同一則同區間只留最新。廣告→貼文靠 Meta 的 `effective_object_story_id`／`effective_instagram_media_id`，貼文→影片靠 `metrics[].postId`，**兩步都是平台給的 id，不猜**；同一則貼文掛在兩支片上就兩支都不給。`clicks` 用 `inline_link_clicks`（點連結，不是點圖）。⚠️ 只有「花了多少」，「賺回多少」要等 Shopline 訂單對回來 |
| `adsAt` | string(ISO) | 廣告花費更新時間 | 後端最後一次寫入的時間 |
...
| `channel` | string | 二創平台別 | `""`＝一般（源片本身）；`"shopee"`＝蝦皮二創版（同語言、換平台）；`"ms"`＝馬來西亞二創版（翻馬來文重剪，比照蝦皮流程、價格換 MYR）；`"remake"`＝**二創**（同語言同平台，原片再剪一次，v201，見下方「二創流程」）。跟 `locale` 是平行的兩種衍生方式 |
...
| `perf` | 影片成效 | boss, manager, editor | `perf` |
| `output` | 剪輯產出 | boss, hr | `output` |
| `attend` | 出勤 | boss, hr | `attend` |
| `df` | 大流量影片 | boss, manager, editor | `videosDF` |
| `lead` | 主管看板 | boss, manager, hr | — |

v204 起沒有 `remake` 這一項了 —— 二創併進「影片成效」，`perf` 的預設值也因此從
`boss` 放寬成原本 `remake` 的範圍（boss／manager／editor）。理由見下方「二創併回影片成效」。
...
`tests/smoke-v202.js` 第 ① 段把改這套**之前**的名單抄在測試裡，逐一比對每一項 × 十個職位。
這裡動一個字，就會有人突然多看到或少看到一整頁，而且不會有任何錯誤訊息。

v204 **刻意**動了兩項（測試裡寫明了為什麼）：`remake` 整個拿掉、`perf` 從 `boss`
放寬成 `boss/manager/editor`。其餘六項一個字都沒變。
...
v203 讓二創自成一頁（`remake`），三張卡擺在一起。老闆看過之後說：

> 「現在的二創理論上不就該和影片成效是一樣的嗎？只是多了一個『二創』的按鍵，
> 還有小主管能夠指派，然後指派後還能新增回原片的影片資料裡面。」

他是對的。二創殼在資料庫裡本來就是一支普通影片（`channel:"remake"` ＋ `sourceVideoId`），
認領、剪輯中、完成、審片、上片、抓成效全部走既有那一套 —— 差別只有那顆按鍵。
所以 v204 把那一頁拆掉，三塊東西各回各家：

| 原本在二創頁的 | 搬到哪 | 為什麼 |
|---|---|---|
| 二創建議 | **影片成效 → 影片排行**，多一個「依二創建議」排法 ＋ 每列一顆「排二創」 | 那張排行本來就是照觀看排的強片榜，跟二創建議是同一批片、同一件事 |
| 剪輯二創成效 | **影片成效**，接在排行下面 | 它就是成效 |
| 進行中的二創 | **看板（流程中控）** | 誰收到了沒、上片日到了還沒人收 —— 這是生產面，不是成效 |

分頁因此從 11 回到 10（`tests/smoke-v181.js` 釘這個數字）。

### 影片排行的三種排法

| 情況 | 池子 | 排序 |
|---|---|---|
| 有搜尋 | **整個影片庫**（不限成效、不限 5,000 觀看） | 照觀看 |
| 依二創建議 | `rmkPool()`：非版本殼 ＋ 觀看 ≥ 5,000 ＋ 排過上片日且日子到了 | 成效 × 隔多久沒用 × 用過幾次 |
| 依觀看（預設） | 有成效數字的片 | 照觀看 |

⚠️ **熱度是百分位，池子一換分數就全變**，所以「依二創建議」一定要用 `rmkRank()` 的預設池，
不能拿全庫去算。搜尋那條線走 `rmkRank(rmkSearchPool(q))`，分數是在搜尋池裡算的 ——
順序換成照觀看，但「為什麼不推薦」照樣標在旁邊（老闆：**只標不擋**）。

### ⚠️ 影片成效本來看不到大流

`viewPerf()` 一直讀 `STATE.videos`，而 `decorate()` 早就把大流那半邊抽到 `videosDF` 去了。
正式資料實測：**大流 47 支裡有 13 支有成效數字，觀看合計 1,467,885** —— 那 146 萬
從來沒被算進平台總覽，那 13 支也從來沒排進影片排行。成效是出片面不是生產面，
v204 改用 `allLibVideos()`（`tests/smoke-v137.js` 的上限因此 6 → 8，理由寫在那裡）。

### ⚠️「上次二創」不可以退回原片的剪輯

`rmkLastCut()` 在「這支從來沒二創過」時會退回原片自己的剪輯 —— 那在原本
「上次誰剪」那一欄是對的，但這一欄叫「上次二創」，印出來會變成「某某剪過這支的二創」，
而他根本沒剪過，旁邊「剪輯」那一欄還印著同一個名字。沒有二創就留破折號。
（這是在真的瀏覽器上看出來的，測試補在 `tests/smoke-v201.js`。）

### ⚠️「大流量」是核心，被移除的只有畫面

老闆特別澄清過：「不是叫你把大流量的移除，那是我們核心要做的事情」。
找出高流量的片拿來再用，正是整套東西的目的。候選池照樣含大流的片
（公司做過的 5 次二創**全部都在大流**）—— 搬走的是**畫面**，不是資料。

### ⚠️「多久沒用／用過幾次」看的是**平台上真正的發文日**（v205）

老闆在畫面上抓到的：一支片在排行寫「多久沒用 —」，點進去卻看到它 8/12 就發過了。
原因是排序只看**人工填的預排上片日**（`scheduledDate`，那支填 2026-10-10，還沒到）。

正式資料實測，168 支有發文日的片：

| | |
|---|---:|
| 排程日與實際發文日對得起來（7 天內） | 100 支 |
| 差超過 7 天 | **47 支** |
| 排程日還在未來，但早就發了 | **7 支** |
| 根本沒填排程日 | **14 支** |

也就是 **68 支的「多久沒用／用過幾次」是錯的** —— 而那正是二創建議排序的兩個乘數。
最誇張的一支：排程說「32 天沒用、用過 1 次」，實際上三個月內重發了 7 次、最近一次
是 4 天前，系統卻把它排在推薦的第 11 名。

所以 `rmkAired()` 把 `metrics[].postAt` 也算進去。改完之後推薦名單上
**13 支掉出去**（其實幾天前才重發過）、**11 支新進來**（排程日還在未來但早就發了）。

**⚠️ 日期去重就夠了，不要做鄰近日期合併。** 同一天的 FB＋IG 自動算一天。量過正式資料：
相隔剛好一天的 7 對裡，只有 1 對是跨平台時差，其他 6 對都是同一個粉專隔天又發一次
（那是真的重發）。為了修 1 個時差吃掉 6 次真重發，划不來。

### ⚠️「同一支片發了 N 次」不是貼文則數

成效卡底下那一行以前寫「同一支片發了 N 次」，N 取的是**貼文則數** ——
但 FB 一則 ＋ IG 一則是同一次上片發到兩個平台，不是發了兩次。
畫面上會出現「發了 2 次」而旁邊排行寫「多久沒用 —」，兩個數字打架。

改成講**出過幾天**（跟 `rmkAired()` 同一個算法），則數另外講：
「這支片出過 **3** 次，共 5 則貼文」／「同一次上片，共 2 則貼文」。

### ⚠️ FB 觀看數：修好之後，舊的壞數字不會自己好

2026-09-12 修好 Reels 播放數之後，正式資料上還有 **12 列「觀看比讚還少」**
（物理上不可能 —— 沒有人能在沒看過的情況下按讚），最誇張的是「觀看 1、讚 182、分享 29」。

兩個不同的原因：

**① 10 列是舊版寫的殘留。** 同步只重問「留言夠多」或「還在追蹤期內」的貼文，
一則一個月前、留言只有 1 的舊貼文永遠不會被重問，就停在錯的數字上。
→ `looks_broken()`：存下來的那一列如果**觀看 < 讚**，或上次標了 `viewsMissing`，
`needs_insights()` 就再問一次。下一次排程同步自己會修，不用任何人記得下指令。

**② 2 列是修好的版本也抓不到。** 它們的網址是 `facebook.com/<粉專>/posts/<id>`，
不是 `/reel/<id>/` —— `RE_FB_VIDEO` 拆不出影片 id，就從頭到尾沒問過影片。
→ `fb_video_id_deep()`：網址拆不出來的時候，**逐則**去問那篇貼文的
`object_id` / `attachments{target}`。只在拆不出來時才問（275 則裡只有 2 則）。
⚠️ 不可以改成在 `/posts` 清單上要 attachments 的子欄位 —— 會被擋成
`(#12) deprecate_post_aggregated_fields_for_attachement`，整支掛掉。

**③ `viewsMissing` 以前會說謊。** 它寫成「FB 而且沒有影片 id → 不算抓不到」，
用意是對的（圖文貼文沒有播放數是正常的，標成抓不到會變成熄不掉的紅字），
但它把**真的沒問到的影片**也一起吞成 0，畫面上看起來就像「這支片真的沒人看」。
→ `viewsMissing` 誠實記（`view is None`），改在**報告那一段**用 `notVideo` 分兩堆講。

### 用過幾次：不歸零

老闆：「用過幾次 不要四個就歸 0，還是能用，只是上面要註明一個小數字，已經用過幾次。」

`rmkUsedK()` 1 次＝1.0、2 次＝0.7、3 次＝0.4，4 次以後每多一次 ×0.6，**下限 0.1**。
永遠排得出來，只是排在後面。次數本身用 `rmkUsedBadge()` 標在片名旁邊（1 次不標，
4 次以上用警示色），讓人自己判斷還要不要再榨這一支。
真正該擋的是「剛用過」（`rmkTimeK()` 的 30 天冷卻）—— 那是時間問題，不是次數問題。
`rmkWhyNot()` 因此也不再把「已經用過 4 次」當成不推薦的理由。

### 老闆的分頁暫時變成 11 個

`tests/smoke-v181.js` 有一條「不要再回到什麼都塞進導覽列」的上限，本來是 10。
加了「二創」變 11 —— 這是**暫時的**，大流量那一頁拿掉之後就回到 10。
⚠️ 下一次有人想加第 12 個之前，先把大流量收掉。
...
**禁止修改**）給指定的剪輯，他們收到（才等同新片有拿到毛片）才開始剪，然後剪完再次
上傳後我才能再回到第一步，再次追蹤。」

二創＝一筆**版本殼**：`channel:"remake"` ＋ `sourceVideoId` 指回原片。
形狀跟蝦皮／馬來／英／泰那四條線一樣，所以排日期、指派、認領計時、審片、完成、
拖延警示、團隊看板整套直接沿用 —— 沒有第二套流程要維護。

跟那四條線只差兩件事：

| | 蝦皮／馬來／英／泰 | 二創 |
|---|---|---|
| 月曆 | 各自一本（`schedLineOf` 回它自己的線） | **跟原片同一本**（`schedLineOf` 往上找原片，通常是 `tw`） |
| 片名 | 自動加後綴（「（蝦皮版）」） | **排片人現取**；`rawName` 沿用原片，上片後鎖住 |
| 誰發起 | 剪輯自己挑源片、指派給自己 | 有**「工作指派」權限**的人排，指派給某個剪輯 |

> **用詞**：二創拿到的不是毛片，是**舊片素材** —— 那是一支已經上過片的成品。
> 講成「毛片」會讓剪輯以為還要等攝影給檔案。畫面上只有「原毛片名」那一欄
> 照舊叫毛片，因為那本來就是原片毛片的名字。

### 誰能排二創：走既有的「工作指派」權限

`canPlanRemake()` 直接回 `canAssignWork()`，不另外發明一套名單。
老闆要的「工作指派」權限系統裡本來就有：`users.canAssign`（設定→成員的勾勾，正式資料
2026-09 是管理員／經理人 ＋ 泓儒）。排二創跟指派毛片本來就是同一件事 ——
把工作分給誰。共用一個開關，換人時只要改一個地方；兩份定義遲早會不一致。

流程：排片人排「上片日期」＋取新片名＋指定剪輯 → 剪輯在「每日工作」按
**認領開始剪**（＝收到毛片，`claimedAt` 從這裡起算）→ 剪完存回同一個資料夾 →
審片 → 上片 → 成效自動回來，原片視窗的「二創」卡並排顯示。
...
就算原片在大流也一樣。生產面（每日工作、待認領、團隊看板）讀的是 `STATE.videos`，
大流那半邊已經被 `decorate` 切到 `videosDF` 去了 —— 殼放大流＝被指派的剪輯永遠看不到
自己有工作。（它照樣不會污染毛片庫存：`isVersion()` 為真，`rawStock()` 只收源片。）

### 原毛片名鎖住不是規矩問題，而且**上片後才鎖**

成效是拿 `videoCopy`／`name`／`rawName` 去跟平台貼文比對的。名字一改，那支片的成效整批
對不回來。所以 `rmkNameLock()` 會回一句鎖定理由，畫面上 `readonly`，**存檔時再擋一次**
（`saveVideo` 回存舊值）—— readonly 擋得住手滑，擋不住 devtools 或一個開了三天的舊分頁。

⚠️ 但**上片日過了才鎖**（老闆：「鎖死，是在影片上架日過後再鎖，以免要修改」）。
上片之前平台上還沒有這支的貼文，沒有東西會對不上，而那正是最需要能改的時候
（打錯字、片名臨時要換）。判斷在 `rmkSelfAired()`：看**它自己**的上片日到了沒
（或已標 `published`／`已上片`），不看它的二創 —— 鎖的理由是「平台上已經有這支的貼文」，
那是它自己的事。

### 二創的二創掛回原片，但**看得出是題材還是剪輯**

`rmkPlanTarget()`：成效永遠拿**原片**當基準。鏈子一節一節接下去，「比原本好還是壞」
會越比越糊塗（第 3 版比第 2 版好 8%、第 2 版比原片差 60%，那第 3 版到底行不行？）。

老闆補的：「如果已經剪了二創 3 次，每次都退步，那就話題不行了，還是剪輯爛。」
`rmkTrend()` 把同一支片每一次的比值排成一條線（`60% → 45% → 30%`）放在原片視窗上，
並且只在形狀夠明顯時才下判斷：

| 形狀 | 判斷 |
|---|---|
| 三次以上、每一次都比上一次低 | 「這個題材到頂了，不是哪個人剪壞」 |
| 不同人剪、最好 ≥50% 且 ≥ 最差 ×1.8 | 「差在剪輯，不是題材」 |
| 其他 | 只排數字，寫「還看不出是題材還是剪輯」 |

兩三個點本來就撐不起結論 —— 形狀不明顯時不要硬給一句話，那只是把雜訊講得像發現。

### 比值旁邊一定要寫原片年齡

老闆選的是「照算，也拿來排剪輯」。正因為這樣，`rmkSrcAgeDays()` 算出來的天數一定要
擺在旁邊，原片累積 155 天、二創才跑 30 天，比值天生就難看 —— 看得到年齡，才分得出
「這個剪輯不行」還是「這支本來就比得不公平」。超過 60 天的原片在卡片下方直接寫白話警語。

### ⚠️「對到很多則」不等於誤配 —— 要看形狀，不是數量（v202）
**疑似誤配的處理方式，三個版本的演進：**

| 版本 | 做法 | 為什麼不夠 |
|---|---|---|
| ① | 警告印在寫入**後面** | 發現問題卻擋不住問題，等於沒有警告 |
| ② | 整支中止（`return 2`） | 為了 2 支可疑的，擋住另外 166 支正確的 |
| ③ 現在 | **可疑的跳過、其餘照寫** | `--force` 才連可疑的一起寫 |

⚠️ 關鍵是「真的被排除」，不是「印一行警告然後照寫」—— 那會退回版本 ①。
測試盯著 `skip` 集合真的被用在組寫入清單的那個迴圈裡。


原本只看則數：超過 8 則就喊誤配、擋住寫入。那個數字是照 734 則那次大磁鐵訂的。
結果一支寵粉商品片在兩個平台、半年內重發 11 次也被喊成誤配 —— 11 跟 734 是兩回事。
...
| | 那堆貼文彼此的文案 |
|---|---|
| **磁鐵** | 都不一樣（幾百個不同商品，只共用一句招呼語） |
| **重發** | 幾乎一模一樣（同一支片、同一段文案，發很多次） |

所以則數超標只是**觸發條件**，接著用 `looks_like_reposts()` 看那幾則彼此的相似度
（兩兩 Jaccard 的中位數），像的就放行。

⚠️ **相似度一定要先扣掉罐頭句再算**。招呼語一長，不扣的話磁鐵會被墊到 0.57 ——
高過門檻，反而被判成重發。扣掉之後：磁鐵 0.37、重發 0.90～1.00，中間空得很開，
門檻怎麼動都不會翻面。`tests/meta-match.py` 有一條專門守這件事。
...
所以 `meta_match` 的索引單位改成**家族**（原片＋它的每一支二創），先認出是哪一家，
再用**上片日期**決定是家族裡的哪一支（`attribute()`，容忍 `FAMILY_NEAR_DAYS=7` 天）。

**這不只是為了認出二創 —— 不這樣做，原片自己的成效會消失。**
拿正式資料實測（一支片掛 2 支二創）：

| | 原片自己那則貼文 | 二創那則 |
|---|---|---|
| 分開索引（舊） | **對不上**（「有 3 支文案一樣、日期也分不開」） | RMK1 |
| 家族歸戶（新） | 原片 ✓ | RMK1（上片日期差 1 天）✓ |

分不出來就算**原片**：寧可少算一筆二創，也不要把不確定的成效掛到某個剪輯頭上 ——
那個數字最後會變成「這個剪輯適不適任」的證據。
上片連結對得上的話贏過日期（那是最硬的證據），而且連結要比對到**家族裡每一支**。

`df`（罐頭句判定）一個家族只數一次。照筆數數的話，一支片被二創 6 次，它的腳本就在
7 筆資料裡各出現一次 → 被當成罐頭句扣掉 → 這支片的指紋沒了，整家的成效全部對不回來。

### `metricsHist` — 成效快照（v201）

```js
metricsHist: [{ postId, d, views, comments }]   // d＝抓取日
```

Meta 只給「到現在為止的累計」，每次同步都把上一次蓋掉。於是原片是半年前上的、累計
10 萬；二創上了 30 天 3 萬 —— 系統會說「只有原本的 30%」。**那句話是假的**：我們根本
不知道原片它自己前 30 天拿多少，搞不好也才 2.5 萬，那這個二創其實是贏的。

這個數字沒有人存過、也補不回來（老片就是沒有），只能從現在開始存：每次同步替還在
追蹤期內的每一則貼文記一個點，上片超過 `HIST_MAX_DAYS=35` 天就不再記，一則最多留
`HIST_MAX_PER_POST=15` 個點。洞察沒給數字的（`viewsMissing`）不記 —— 空的不是 0。

被二創過的原片（`remake_parents()`）一律繼續量，留言再少也要量：原片的數字停在半年前、
二創的數字是這個月的，那個比值就不是在比剪輯，是在比誰的數字比較新。

> **老片的比值怎麼看**：老闆選的是「照算，也拿來排剪輯」。所以比值照算、照排，
> 但每一列都把原片的年齡寫出來（`rmkSrcAgeDays`）。等新片累積兩三個月，
> `metricsHist` 就有「第 30 天對第 30 天」可以比，那時候的比值才是真的。

### 看結果：成效頁的「剪輯二創成效」

`rmkCompare(二創)` 回 `{r, basis, age}`：兩邊都有第 30 天的點就回 `basis:"same"`（真的比值），
否則退回累計並帶出原片年齡（`basis:"total"`）。畫面上兩種都會標出來是哪一種。

`rmkViewsAtAge(v, 30)` 要**每一則貼文都有**第 30 天的點才算數 —— 一支片在兩個帳號各發
一次，只拿到其中一則就去比，等於拿半支片比整支片，那比沒有數字更糟。

剪輯排行用**中位數**不用平均：一支爆片就能把一個人的平均拉到天上，那不叫穩定。
還沒有數字的算「進行中」，不進比值（沒有數字不等於零分）。另外一欄是「他分到的原片
平均幾天」—— 有人平均分到 150 天的老片，比值難看不一定是他的問題，這一欄就是要讓
分配的人看見這件事。

### 二創算一次「再用」

`rmkAired(v)` 把它的二創的上片日期也算進去（還沒到上片日的不算）。少了這一段，一支片
被二創三次之後，建議選單上還是寫「用過 1 次、120 天沒用」，然後它會一直排在最前面被
推薦去二創第四次 —— 跟老闆說的「也不能太常用」正好相反。

`remakesOfSrc()` 走 `rmkIndex()` 建的表（快取靠陣列身分認，跟 `allLibVideos` 同一招）：
排序那裡是「每支片都問一次」，掃全庫的話 1,045 支就變成一百萬次比對。

⚠️ **但有搜尋的時候，一律照觀看排，不照二創分數排。**
老闆：「他還是要能被排列出來（如果我要找強片為了某個品項銷售）。」
有搜尋＝他在找「這個商品最強的那支片」，不是在找「現在最適合二創的片」。
照分數排的話，一支二創過三次的 10 萬觀看強片會沉到一堆沒人看過的新片下面。
二創狀態照樣標在旁邊（`rmkWhyNot`：「18 天前才用過」「已經用過 4 次」）—— **只標不擋**。

### ⚠️「上次誰剪」與「上次二創幾成」必須指同一個人

第一版兩個值各自取，畫面上就出現「巧芸　上次二創 50%」—— 那 50% 是陳鋒剪的，
巧芸那支還在剪。同一格裡兩個數字指到不同的人，看的人一定誤會。
`rmkLastCut()` 一次回傳 `{who, c}`：有成績就都取最近一支**有成績的**，都還沒有成績
就只顯示最近被指派的人、不給數字。
