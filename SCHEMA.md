...
| `role` | string | `boss`（管理員）／`manager`（經理人）／`editor`（剪輯）／`intl`（海外剪輯・全英文介面）／`hr`（人資：只看每個人的今日／本月成效，純檢視、不能操作） |
...
> **人資（`role:"hr"`）員工成效（v64 起）**：只有一個分頁「員工成效」，**純檢視、沒有任何按鍵**，
> 也不會寫入任何資料。畫面只有兩塊 ——
> ①**今日成效**：每人一張卡（出勤時間與工時、今日完成、進行中、交辦完成 x/y、完成片名與交辦回報）。
> ②**本月成效**：一張表（完成上架、剪片速度＝認領到完成的平均天數、平均工時、帶商品數、出勤天數、交辦完成）。
> 全部由 `videos`／`shifts`／`tasks` 即時算出來，人資看不到也按不到任何會動到影片的東西。
