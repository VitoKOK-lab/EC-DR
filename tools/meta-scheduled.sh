...
#
# v226（老闆指定）：改成每天半夜同步一次。以前是每天叫起來、腳本自己判斷
# 上次成功是不是三天內（EC_DR_META_EVERY=3）——這個「每天醒來、腳本自己判斷」
# 的機制本身留著：EVERY 現在是 1，意思是「今天已經成功跑過就不要重複跑」，
# 還是能擋住 launchd 同一天叫兩次的情況；哪天又要改成每 N 天，只要調這個數字，
# 不用再去動 launchd 的排程設定。
# =============================================================================
set -uo pipefail
