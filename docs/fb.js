...
if (!firebaseConfig || String(firebaseConfig.apiKey || "").includes("PASTE")) {
  ...
  const raw = { users: [], videos: [], schedule: {}, settings: {}, tasks: {}, shifts: {}, products: {} };
  ...
  onAuthStateChanged(auth, async (user) => {
    ...
    if (!snap.exists()) { await setDoc(sref, DEFAULT_SETTINGS); }
      ...
      if (cur.schemaVersion == null || cur.schemaVersion < 9) patch.schemaVersion = 9;
    ...
    onSnapshot(collection(db, "products"), q => { const s = {}; q.docs.forEach(d => s[d.id] = d.data()); raw.products = s; push(); });
  });
}
