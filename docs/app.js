...
// 階段只有三種（含相容舊資料）：毛片待剪 → 剪輯中 → 已上架
const STAGE_TODO="毛片待剪", STAGE_EDIT="剪輯中", STAGE_DONE="已上架";
const STAGES=[STAGE_TODO, STAGE_EDIT, STAGE_DONE];
function isPending(v){ const s=v&&v.stage; return !s || s===STAGE_TODO || s==="待處理"; }
function isEditing(v){ return !!v && v.stage===STAGE_EDIT; }
function isDoneStage(v){ const s=v&&v.stage; return s===STAGE_DONE || s==="已完成" || s==="已上片"; }
function stageLabel(v){ return isDoneStage(v)?STAGE_DONE:(isEditing(v)?STAGE_EDIT:STAGE_TODO); }
...
function dayVideoList(date){
  ...
  (STATE.videos||[]).forEach(v=>{ if(v.scheduledDate===date && isDoneStage(v) && !seen.has(v.id)){ seen.add(v.id); out.push({videoId:v.id, fromVideo:true}); } });
  return out;
}
...
// 我目前進行中的影片數
function inProgressCount(name){ return (STATE.videos||[]).filter(v=>isEditing(v)&&(v.claimedBy===name||v.editor===name)).length; }
...
async function route(method, path, body){
  ...
  if(head==="videos"){
    ...
    if(action==="finish"){
      ...
      const patch={stage:STAGE_DONE,finishedAt:nowIso(),editor:ed,locked:true,updatedAt:nowIso(),
...
function openDay(ds){
  ...
  const doneList=(STATE.videos||[]).filter(v=>isDoneStage(v) && !usedIds.has(v.id) && vidIsOld(v))  // 過了預排上片日（舊片）才能重播
...
function viewWork(){
  ...
  const mine = (STATE.videos||[]).filter(v=>(v.claimedBy===me||v.editor===me) && isEditing(v))
    .sort((a,b)=>String(a.claimedAt||"").localeCompare(String(b.claimedAt||"")));
  const pool = (STATE.videos||[]).filter(v=>isPending(v));
...
function clockOutReport(){
  ...
  const wip=(STATE.videos||[]).filter(v=>(v.claimedBy===me||v.editor===me) && isEditing(v));
  const tasks=myTasks();
  const body=`
...
// 「其他標籤」= 設定的標籤清單，去掉自成一類的 新片/舊片（新舊由分頁／預排日代表）
...
function isPublished(v){ return !!(v && (v.published===true || isDoneStage(v))); }
...
let VID_VIEW="new";       // 影片庫分頁：new=新片待發(預排日未到) / old=舊片(已過預排日) / all=全部
let VID_TAGS=new Set();   // 標籤篩選（可複選）
// 一列 = 一支影片
function vidTableRow(v){
  const stageCol={[STAGE_TODO]:"var(--muted)","待處理":"var(--muted)",[STAGE_EDIT]:"var(--accent)",[STAGE_DONE]:"var(--green)","已完成":"var(--green)","已上片":"var(--green)"}[v.stage]||"var(--muted)";
  ...
  return `<tr onclick="editVideo('${v.id}')" style="cursor:pointer">
    ...
    <td data-label="狀態"><span class="ststack">
      <span class="pill" style="font-size:11px;background:transparent;border:1px solid ${stageCol};color:${stageCol}">${esc(stageLabel(v))}</span>
      ${rev}</span></td>
  </tr>`;
}
function vidRowsHTML(){
  ...
  // 待發在前、依最後更新日新到舊
  const rank={[STAGE_TODO]:0,"待處理":0,[STAGE_EDIT]:1,[STAGE_DONE]:2,"已完成":2,"已上片":3};
...
// 影片內容：預設觀看模式（不可改），右上「✎ 編輯」才進編輯；右上「✕」關閉
...
function openVideoModal(id, edit){
  ...
  const body=`
    ...
    <div class="grid cols2">
      ...
      <div><label>階段</label><select id="e_stage">${STAGES.map(c=>`<option ${stageLabel(v)===c?"selected":""}>${esc(c)}</option>`).join("")}</select></div>
    </div>
